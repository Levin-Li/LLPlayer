...
- (void) setFrame: (LLVideoFrame *) frame;
- (BOOL) prepareRender;
@end
...
- (void) setFrame: (LLVideoFrame *) frame
{
    LLVideoFrameYUV *yuvFrame = (LLVideoFrameYUV *)frame;
...
- (id) initWithFrame:(CGRect)frame videoWidth:(float)width videoHeight:(float)height
{
    ...
    if (self) {
        ...
        _videoWidth = width;
        _videoHeight = height;
        NSLog(@"dispaly width=%f height=%f",width,height);
        if (1) {
...
- (void)updateVertices
{
    ...
    const float width   = _videoWidth;
    const float height  = _videoHeight;
