...
@interface LLAudioOutPutQueue()
{
    
}
@end

@implementation LLAudioOutPutQueue
{
...
{
    LLAudioOutPutQueue *audio = (__bridge LLAudioOutPutQueue *)inUserData;
    if (audio.receiveData.count >0) {
        NSData *pcmData = [audio.receiveData objectAtIndex:0];//每个4096
        NSLog(@"播放数据长度为=%lu buffsize=%d",(unsigned long)pcmData.length,audio.bufferSize);
        if (pcmData) {
            
            if (pcmData.length <= audio.bufferSize) {
                NSLog(@"缓冲区buffer个数:%d",(int)audio.receiveData.count);
            ...
            if (audio.receiveData.count) {
                [audio.receiveData removeObjectAtIndex:0];
            }
